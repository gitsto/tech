...
class ForSaleState implements IPropertySquareState {
    ...
    public void landOnBy(PropertySquare s, Player p) {
	   // re-write this method to use a strategy.
    }
}
...
class OwnedState implements IPropertySquareState {
	...
	if (p != s.getOwner()) {
	    ...
	    System.out.println(p + " pay " + s.getRent() + " to " + s.getOwner() + " on square " + s);
	    System.out.println(p + " credit = " + p.getMoney());
	    System.out.println(s.getOwner() + " credit = " + s.getOwner().getMoney());
	}
    }
}
