...
public class PhiloGUI1 {
	...
	public void reflechir(int numero) {
		try {
			SwingUtilities.invokeAndWait(new TextFieldUpdate(txtReflechir, Color.yellow,"philo " + numero + " reflechit"));
		} catch (InterruptedException e) { e.printStackTrace();
		} catch (InvocationTargetException e) { e.printStackTrace(); }
	}
		
	public void discuter(int numero) {
		try {
			SwingUtilities.invokeAndWait(new TextFieldUpdate(txtReflechir, Color.pink,"philo " + numero + " discute"));
		} catch (InterruptedException e) { e.printStackTrace();
		} catch (InvocationTargetException e) { e.printStackTrace(); }
	}
	...
	public void nePasManger(int numero) {
		try {
			SwingUtilities.invokeAndWait(new TextFieldUpdate(txtManger, Color.pink,"philo " + numero + " ne mange pas ..."));
		} catch (InterruptedException e) { e.printStackTrace();
		} catch (InvocationTargetException e) { e.printStackTrace(); }
	}
